...
public class WebShopApplication implements CommandLineRunner {
    public static final String BASE_URL_CUSTOMERS = "http://localhost:8081";
    public static final String BASE_URL_PRODUCTS = "http://localhost:8082";
    public static final String BASE_URL_CART = "http://localhost:8083";
    public static final String BASE_URL_ORDERS = "http://localhost:8084";
    @Autowired
    ...
    public void run(String... args) throws Exception {
        ...
        restTemplate.postForLocation(BASE_URL_CUSTOMERS + "/customer", customerDto);

        // get customer
        //call the customer component to get the customer with id 101 and print the result
        ResponseEntity<CustomerDTO> customer = restTemplate.getForEntity(BASE_URL_CUSTOMERS + "/customer/101", CustomerDTO.class);
        ...
        restTemplate.postForLocation(
                String.format("%s/product/%s/%s/%.2f", BASE_URL_PRODUCTS, "101", "The first product", 31.99), null);
        //call the product component to create the second product
        restTemplate.postForLocation(
                String.format("%s/product/%s/%s/%.2f", BASE_URL_PRODUCTS, "102", "The second product", 13.49), null);

        //set stock
        //call the product component to set the stock for the first product
        restTemplate.postForLocation(
                String.format("%s/product/stock/%s/%d/%s", BASE_URL_PRODUCTS, "101", 7, "Denver"), null);

        //get a product
        //call the product component to get the the first product and print the result
        ResponseEntity<ProductDTO> productOne = restTemplate.getForEntity(BASE_URL_PRODUCTS + "/product/101", ProductDTO.class);
        ...
        String cartId = "1";
        restTemplate.postForLocation(BASE_URL_CART + "/cart/" + cartId + "/101/1", null);
        //call the shopping component to add the second product to the cart
        restTemplate.postForLocation(BASE_URL_CART + "/cart/" + cartId + "/102/1", null);

        //get the shoppingcart
        //call the shopping component to get the cart and print the result
        ResponseEntity<ShoppingCartDTO> cartResponse = restTemplate.getForEntity(BASE_URL_CART + "/cart/" + cartId, ShoppingCartDTO.class);
        ...
        restTemplate.postForLocation(BASE_URL_CART + "/cart/checkout/" + cartId, null);

        //get the order
        //call the order component to get the order and print the result
        String orderId = cartId;
        ResponseEntity<OrderDTO> orderResponse = restTemplate.getForEntity(BASE_URL_ORDERS + "/order/" + orderId, OrderDTO.class);
        orderResponse.getBody().print();

        //add customer to order
        //call the order component to add a customer to the order
        restTemplate.postForLocation(BASE_URL_ORDERS + "/order/setCustomer/" + orderId + "/101", null);

        //confirm the order
        //call the order component to confirm the order
        restTemplate.postForLocation(BASE_URL_ORDERS + "/order/" + orderId, null);

        //get the order
        //call the order component to get the order and print the result
        orderResponse = restTemplate.getForEntity(BASE_URL_ORDERS + "/order/" + orderId, OrderDTO.class);
        orderResponse.getBody().print();
